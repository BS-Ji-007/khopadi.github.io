...
const Movies = () => {
  ...
  const loadMovies = async () => {
    setLoading(true);
    const data = await tmdb.movies(page);
  ...
  return (
    <div className="min-h-screen pt-24 pb-10" style={{ background: 'var(--bg-deep)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="mb-8">
          <h1 className="font-display text-5xl text-white tracking-wide mb-2">POPULAR MOVIES</h1>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Discover the most popular movies right now</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-20 text-center">
            <p className="text-xl" style={{ color: 'var(--text-secondary)' }}>No movies found</p>
          </div>
        )}
      </div>
    </div>
